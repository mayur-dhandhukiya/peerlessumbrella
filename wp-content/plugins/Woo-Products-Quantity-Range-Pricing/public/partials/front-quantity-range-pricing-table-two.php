 ...
 */
$tab_header        = '';
$tab_price_tab_one = $tab_price_tab_one_2 = $tab_price_tab_three_2 = $tab_price_tab_two = $tab_price_tab_two_2 = $tab_price_tab_one_sec = $tab_price_tab_three = $tab_price_tab_three_sec = $regular_price = $sales_price = $regular_price_td = $sales_price_td = '';
$tab_price         = 0;
?>
	...
	foreach ( $as_quantity_rage_values as $as_quantity_rage_v ) {
		...
		if ( is_array( $as_quantity_rage_v ) ) {
			...
			$as_quantity_rage_value = array(
				...
				'sec_cnd_price' => $as_quantity_rage_v->sec_price,
		...
		if ( ! empty( $as_quantity_rage_value['min_qty'] ) && $as_quantity_rage_value['max_qty'] && $as_quantity_rage_value['price'] && $data_show != false ) {
            ...
            $sec_cnd_price = $as_quantity_rage_value['sec_cnd_price']; ?>
					...
					$sec_cnd_price = str_replace( $decimal_separator, ".", $sec_cnd_price );
					$sec_cnd_price = str_replace( $thousand_separator, "", $sec_cnd_price );
					...
					if(is_numeric($sec_cad_price_display)) {
						$price_d_sec         = ( $sec_cad_price_display * 1.32 );
					} else {
						$price_d_sec = 0;
					}
					...
					$tab_price_tab_one_2 .= '<td> ' . $sec_price_display . ' </td>';
					$tab_price_tab_two_2 .= '<td> ' . $sec_cad_price_display . ' </td>';
					$tab_price_tab_three .= '<td> ' . number_format( (float) $price_d, 2, '.', '' ) . ' </td>';
					$tab_price_tab_three_2 .= '<td> ' . number_format( (float) $price_d_sec, 2, '.', '' ) . ' </td>';
...
<div class="as-tabs tabs variation_quantity_table 77777777" style="order:5 !important;display: block !important;">
    <ul class="nav nav-tabs ">
        <li class="nav-item english-nav active english-nav"><a href="#tab-one" id="tab-one-title" class="nav-link"
                                                               data-toggle="tab">US Pricing Decorated</a></li>
        <li class="nav-item english-nav english-nav"><a href="#tab-one-2" id="tab-one-title" class="nav-link"
                                                        data-toggle="tab">US Pricing Blank</a></li>
        <li class="nav-item franch-nav"><a href="#tab-two" id="tab-two-title" class="nav-link" data-toggle="tab">CND
                Program Decorated</a></li>
        <li class="nav-item franch-nav"><a href="#tab-two-2" id="tab-two-title" class="nav-link" data-toggle="tab">CND
                Program Blank</a></li>
        <li class="nav-item franch-nav"><a href="#tab-three" id="tab-three-title" class="nav-link" data-toggle="tab">CND
                Currency Decorated</a></li>
        <li class="nav-item franch-nav"><a href="#tab-three-2" id="tab-three-title" class="nav-link" data-toggle="tab">CND
                Currency Blank</a></li>
    </ul>
    <div class="tab-content">
        ...
        <div id="tab-three" class="tab-pane fffffff">
            <div class="wpb_text_column wpb_content_element ">
                <div class="wpb_wrapper">
                    <table class="table" width="100px">
                        <tbody>
        ...
        <div id="tab-three-2" class="tab-pane">
            <div class="wpb_text_column wpb_content_element ">
                <div class="wpb_wrapper">
                    <table class="table" width="100px">
                        <tbody>
                        ...
                        <tr>
                            <td><?php echo $tab_price.$code_a; ?></td>
						    <?php echo $tab_price_tab_three_2; ?>
                        </tr>
                        <?php 
                        if(!empty($regular_price_td)){
                        	?>
    ...
    <div>
    	<?php echo get_post_meta( $product->get_id(), 'after_price_box_tex', true ); ?>
    </div>
</div>