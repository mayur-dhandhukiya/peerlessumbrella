 ...
 */
abstract class UYDGoogle_Logger_Abstract
{
  /**
   * Default log format
   */
  ...
  /**
   * @param UYDGoogle_Client $client  The current Google client
   */
  public function __construct(UYDGoogle_Client $client)
  {
    $this->setLevel(
        $client->getClassConfig('UYDGoogle_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('UYDGoogle_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('UYDGoogle_Logger_Abstract', 'date_format');
   ...
   * @throws UYDGoogle_Logger_Exception If $level is invalid
   */
  protected function normalizeLevel($level)
  {
    ...
    throw new UYDGoogle_Logger_Exception(
