 ...
 */
class UYDGoogle_Logger_File extends UYDGoogle_Logger_Abstract
{
  /**
  ...
  public function __construct(UYDGoogle_Client $client)
  {
    ...
    $file = $client->getClassConfig('UYDGoogle_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('UYDGoogle_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('UYDGoogle_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new UYDGoogle_Logger_Exception('File pointer is no longer available');
    }
