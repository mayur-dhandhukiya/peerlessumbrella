...
/**
 * Curl based implementation of UYDGoogle_IO.
 *
...
class UYDGoogle_IO_Curl extends UYDGoogle_IO_Abstract
{
  ...
  public function __construct(UYDGoogle_Client $client)
  {
    if (!extension_loaded('curl')) {
      ...
      throw new UYDGoogle_IO_Exception($error);
    }
   ...
   */
  public function executeRequest(UYDGoogle_Http_Request $request)
  {
    ...
    if (defined('FORCE_CURL_HTTP_VERSION_1_1') && FORCE_CURL_HTTP_VERSION_1_1 === true){
        // Prevent 'Uncaught UYDGoogle_IO_Exception: HTTP/2 stream 0 was not closed cleanly: PROTOCOL_ERROR (err 1)' errors
    ...
    $options = $this->client->getClassConfig('UYDGoogle_IO_Curl', 'options');
    if (is_array($options)) {
    ...
    if ($response === false) {
      ...
      $map = $this->client->getClassConfig('UYDGoogle_IO_Exception', 'retry_map');

      $this->client->getLogger()->error('cURL ' . $error);
      throw new UYDGoogle_IO_Exception($error, $code, null, $map);
    }
  ...
  public function setDefaultOptions()
  {
    ...
    $timeout = $this->client->getClassConfig('UYDGoogle_IO_Abstract', 'request_timeout_seconds');
    if ($timeout > 0) {
  ...
  protected function needsQuirk()
  {
    ...
    return $versionNum < UYDGoogle_IO_Curl::NO_QUIRK_VERSION;
  }
}
