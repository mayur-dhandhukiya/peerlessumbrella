...
class Helpers
{
    ...
    public static function check_user_role($roles_to_check = [], $user = null)
    {
        if (!is_array($roles_to_check)) {
            Helpers::log_error('Invalid roles set. Expected and array or roles, but got something else.', 'Helpers', ['roles' => $roles_to_check], __LINE__);
    ...
    public static function get_user_location($ip = null)
    {
        ...
        $location = wp_cache_get('wpcp-geolocation-'.$userip, 'wpcp-'.CORE::$slug.'-other', false);

        if (empty($location)) {
            try {
                $geolocation = json_decode(file_get_contents('https://freeipapi.com/api/json/'.$userip), true);

                if (false !== $geolocation && isset($geolocation['ipVersion']) && '-' !== $geolocation['countryName']) {
            ...
            \wp_cache_set('wpcp-geolocation-'.$userip, $location, 'wpcp-'.CORE::$slug.'-other', 300);
        }
    ...
    public static function select_callback($callback)
    {
        switch ($callback) {
            case 'wpcp_uyd_cf7_add_content':
            case 'wpcp_uyd_elementor_add_content':
            case 'wpcp_uyd_gf_add_content':
            case 'wpcp_uyd_wc_add_content':
            case 'wpcp_add_content_to_mce':
            case 'uses_listener':
                break;

            default:
                preg_match('/([0-9a-f]{8}\-[0-9a-f]{4}\-4[0-9a-f]{3}\-[89ab][0-9a-f]{3}\-[0-9a-f]{12})/', $callback, $uuid);
                if (isset($uuid[1])) {
                    $callback = 'wpcp-callback-'.$uuid[1];
                } else {
                    $callback = '';
                }
        }
    ...
    {
        $list = wp_cache_get('wpcp-users-and-roles', 'wpcp', false);
        ...
        foreach ($users as $wp_user) {
            ...
            $list[] = [
                ...
                'text' => esc_html(htmlspecialchars(str_replace('"', '', empty($wp_user->display_name) ? $wp_user->user_login : $wp_user->display_name), ENT_QUOTES)),
                'type' => 'user',
        ...
        wp_cache_add('wpcp-users-and-roles', $list, 'wpcp', 300);
    ...
    public static function return_bytes($size_str)
    {
        ...
        if (!preg_match('/[a-z]/i', str_replace('B', '', $size_str))) {
            return (int) $size_str;
        }
    ...
    {
        Helpers::is_deprecated('function', 'apply_placeholders()', '\TheLion\UseyourDrive\Placeholders::apply()');

        return Placeholders::apply($value, $context, $extra);
    }

    public static function get_iconset_url()
    {
        if (!defined('USEYOURDRIVE_ICON_SET')) {
            define('USEYOURDRIVE_ICON_SET', Settings::get('icon_set'));
        }

        return USEYOURDRIVE_ICON_SET;
    }
    ...
    public static function get_default_thumbnail_icon($mimetype)
    {
        ...
        return Helpers::get_iconset_url().$thumbnailicon;
    }
    ...
    {
        Helpers::log_error("{$type} {$name} is deprecated.".(($replaced_with) ? " Use {$replaced_with} instead." : ''), 'Deprecated', null, __LINE__);
    }
    ...
    public static function get_mimetype($extension = '')
    {
        ...
        $mime_types_map = apply_filters('useyourdrive_set_mimetypes', $mime_types_map);
    ...
    /**
     * Very rudemental function to get the browser name from the user agent.
     *
    ...
    public static function get_browser_name($user_agent = null)
    {
        ...
        if (!empty($browser) && isset($browser['browser'])) {
    ...
    public static function purge_cache_others()
    {
        try {
            ...
            Helpers::log_error('Cannot clear cache.', 'Helpers', null, __LINE__, $ex);
        }
    }

    public static function log_error($message, $kind = 'General', $data = [], $line = null, $exception = null)
    {
        if (!defined('WP_DEBUG') || WP_DEBUG === false) {
            return;
        }

        // Trim the message to a reasonable length (e.g., 1000 characters)
        $maxLength = 1000;
        ...
        // Escape newlines to prevent log splitting
        $message = str_replace(["\r", "\n"], ['\r', '\n'], $message);

        //  Wrap the message in a context for clarity (e.g., JSON format)
        $log = [
            'kind' => $kind,
            'line' => $line,
            'message' => $message,
        ...
        // Add additional data if availalbe
        if (!empty($data)) {
            $log['data'] = $data;
        }

        // Add page information when available:
        if (!empty($page_url = self::get_page_url())) {
            $log['page_url'] = htmlspecialchars($page_url, ENT_QUOTES | ENT_HTML5);
        }

        // Add Exception
        if (is_wp_error($exception)) {
            $log['exception'] = $exception->get_error_message();
        } elseif ($exception instanceof \Exception) {
            $log['exception'] = $exception->getMessage();
        }

        $log_entry = json_encode($log, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);

        error_log("[WP Cloud Plugin message]: {$log_entry}");
    }
}
