(function ($) {
    ...
    $('#nf-builder').on('click', '.useyourdrive.open-shortcode-builder', function () {
        var input_field = $('#wpcp_useyourdrive_shortcode');
    ...
    function callback_handler(event) {
        ...
        $('.wpcp_data_input').val(event.data.shortcode);
        window.modal_action.close();
        $('#wpcp-modal-action.UseyourDrive').remove();

        $('.wpcp_data_input').trigger('change');
