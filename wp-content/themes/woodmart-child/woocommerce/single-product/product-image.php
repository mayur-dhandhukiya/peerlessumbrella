...
$gallery_settings = get_field( 'gallery_settings', $product->get_id() );
...
?>
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?> desktop_slider" data-columns="<?php echo esc_attr( $columns ); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;">
	<figure class="woocommerce-product-gallery__wrapper">
       ...
       // do_action( 'woocommerce_product_thumbnails' );
		?>
		<div class="wd-carousel-wrap" style="cursor: grab; transform: translate3d(0px, 0px, 0px);">
			<?php 
			if( $gallery_settings ){
				foreach( $gallery_settings as $gallery_key => $gallery_val ){
					?>
					<div class="wc-carousel-item">
						<?php 
						if( isset( $gallery_val['gallery_type'] ) && !empty( $gallery_val['gallery_type'] ) ){
							if( $gallery_val['gallery_type'] == 'image' ){
								echo '<a class="wc-carousel-image" href="' . esc_url( $gallery_val['gallery_image'] ) . '"><img src="' . esc_url( $gallery_val['gallery_image'] ) . '" alt="Image" /></a>';
							}else{
								if (  $gallery_val['gallery_image'] ) {
									?>
									<div class="woocommerce-product-gallery__video">
										<a class="wc-carousel-video" href="<?php echo esc_url( $gallery_val['gallery_image'] ); ?>">
											<video autoplay muted loop>
												<source src="<?php echo esc_url( $gallery_val['gallery_image'] ); ?>" type="video/mp4">
	...
	<div class="wp_popup_image mobile_slider"><div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?> images image-action-<?php echo esc_attr( $image_action ); ?>">
